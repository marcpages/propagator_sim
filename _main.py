...
def main():
    ...
    ros_model_code = d.get(
        ROS_MODEL_TAG, WANG_TAG
    )  # switch per scegliere se usare il modello di Rothermel (rothermel), Wang (wang) oppure il classico Propagator (default)
    ...
    if ros_model_code not in [DEFAULT_TAG, WANG_TAG, ROTHERMEL_TAG]:
        logging.info(
            'WARNING: RoS function is not well defined, the model will use "wang" configuration'
        )
    ...
    waterline_actions_fixed = d.get(
        WATERLINE_ACTION_TAG, None
    )  # waterline actions means fire fighting actions made by the use of water
    if waterline_actions_fixed == 0:
        waterline_actions_fixed = None
    waterline_actions = None
    heavy_actions_fixed = d.get(
        HEAVY_ACTION_TAG, None
    )  # heavy actions means fire fighting operations that act on the vegetation (use of earth-moving machines or firebreaks in general)
    if heavy_actions_fixed == 0:
        heavy_actions_fixed = None
    heavy_actions = None
    canadair_fixed = d.get(
        CANADAIR_TAG, None
    )  # canadair means fire fighting actions made by canadairs
    if canadair_fixed == 0:
        canadair_fixed = None
    canadair = None
    helicopter_fixed = d.get(
        HELICOPTER_TAG, None
    )  # helicopter means fire fighting actions made by helicopters
    ...
    if args.do_spot is not None:
        ...
        logging.info(
            "The spotting model flag from command line is..." + str(do_spotting)
        )

    if SPOT_FLAG_TAG in d:
        do_spotting = d.get(SPOT_FLAG_TAG, False)
        logging.info(
            "The spotting model flag from parameter file  is..." + str(do_spotting)
        )
    ...
    easting, northing, zone_number, zone_letter, polys, lines, points = (
        sim.load_ignitions_from_string(ignition_string)
    )
    (
        easting_ign,
        northing_ign,
        ...
        lines_ign,
        points_ign,
    ) = sim.load_ignitions_from_string(ignition_string_begin)
    ...
    sim.set_ignitions(polys_ign, lines_ign, points_ign, zone_number_ign)
    sim.run()
